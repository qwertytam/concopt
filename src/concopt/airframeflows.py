...
class AirframeFlows:
    ...
    @staticmethod
    def saint_venant_formula(q_c, p, y=Phys.gamma_air.magnitude):
        """Saint-Venant formula for calibration of Mach meters at subsonic
        speeds.
        
        Used when q_c/p < 0.893 i.e., at subsonic speeds
        
        q_c can also be written as Delta_p. It is the difference between
        total pressure and static ambient presssure.

        Args:
            q_c (float): impact or dynamic pressure
            p (float): static ambient pressure
            y (float): ratio of specific heats
        ...
        exponent = yless1 / y
        pratio = q_c / p
        M = np.sqrt(2/yless1 * ((pratio + 1)^exponent - 1))
        return M


    @staticmethod
    def reverse_saint_venant_formula(M, y=Phys.gamma_air.magnitude):
        """Saint-Venant formula reversed with M as input and impact pressure as
        output.
        
        Used when M < 1 i.e., at subsonic speeds
        ...
        yless1 = y - 1
        exponent = y / yless1
        pratio = (1 + yless1/2 * M)^exponent - 1
        return pratio
